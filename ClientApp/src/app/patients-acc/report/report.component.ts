...
export class ReportComponent implements OnInit {
  ...
  getReport() {
    ...
    let sub = this.dataService.getPatientsCumulativeReport(model).subscribe(res => {
      this.list = res;
    });
    this.subscriptions.push(sub);
  }

  getDetails(item: IPatientCumulative) {
    let sub = this.dataService.getPatientDetailsReport(item).subscribe(res => {
